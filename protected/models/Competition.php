  ...
  //вернуть модель Соревнование (ИД берётся пока что из хардкода)
    ...
    public static function sqlStat() {
        ...
        $sqlCommand = Yii::app()->db->createCommand(
            ...
            ' UNION '.
            'SELECT "Количество спортсменов", (SELECT COUNT(*) FROM sportsmen S LEFT JOIN command C ON S.commandid = C.commandid WHERE S.status = 1 AND EXISTS ('.
                '(SELECT 1 from proposal P WHERE P.commandid = C.commandid AND P.status = :status))) FROM competition T WHERE T.id = :competitionid');
        return $sqlCommand;
    }
}